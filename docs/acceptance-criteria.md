...
当前状态：已通过。脱敏 `Supplier` 快照生成 get/create/update 工具，自动化测试覆盖必填、只读、掩码和禁止字段。

### AC-MCP-01 协议一致性

同一工具通过 LangGraph 内部调用和 MCP 调用时使用相同 Registry 定义，并产生一致的授权、审计和幂等结果。

当前状态：协议路径已通过。官方 MCP Client 与内部调用复用同一 Registry 和幂等记录；待第二 Agent 接入生成工具后完成业务复用证明。

### AC-RN-01 生命周期恢复

RN 客户端订阅 Run 后切后台，期间产生事件；恢复前台后从最后 sequence 补发，不重复创建任务或展示重复事件。

当前状态：已通过。自动化测试覆盖后台断开、前台补发、重复事件去重、持久化恢复、RN AppState/浏览器生命周期适配和 Session 内不重复创建；Web 实验台通过真实 Contract Agent、审批 API 与 SSE 可视化验证完整链路。真机系统杀进程与厂商后台策略不在当前声明范围内。
...
| AC-WEB-01 | 通过 | 浏览器完成创建、查看 Finding/Evidence、审批和整改写回 |
| AC-WEB-02 | 通过 | Event Projector 对重叠、重复和乱序事件按 sequence 重建投影 |
| AC-WEB-03 | 通过 | `/runs/:runId` 刷新后恢复快照、历史时间线和 SSE 游标 |

当前结论：可以表述为“完成项目风控 Agent、RAG 与 Web 人机协作闭环”；固定评测集尚未完成，不能宣称 Recall、引用正确率、真实延迟或成本指标达标。

## 10. M2 E0 评测骨架验收记录（2026-08-20）

| 验收项 | 状态 | 自动化证据 |
| --- | --- | --- |
| 数据版本化 | 通过 | `retrieval.v1`、`risk-cases.v1`、`tenant-attacks.v1` 均通过 Zod Schema |
| 风控路径 | 通过 | 6 个确定性案件覆盖一次完成、二轮补证、三轮终止、非法计划、无风险完成和审批写回 |
| 检索计算 | 通过 | 8 个问题生成 Recall@5 明细与总体指标 |
| 引用计算 | 通过 | Finding 引用存在性和按风险维度标注的 Evidence 支持关系分别计算 |
...
## 12. M4 面试交付验收记录（2026-08-20）

| 验收项 | 状态 | 验收证据 |
| --- | --- | --- |
| 零密钥一键演示 | 通过 | `pnpm demo -- --api-port=3105 --web-port=5195` 自动启动 API/Web，创建 3 个场景并完成 1 个合同审批 |
| 完整基础设施演示 | 通过 | `pnpm demo:full -- --api-port=3110 --web-port=5200` 自动启动 Docker、迁移、PostgreSQL/Qdrant、API/Web |
| 固定数据幂等 | 通过 | 完整模式第二次启动输出 `0 created, 3 reused, 0 approved`；自动化测试覆盖首次与重复执行 |
| 标准演示材料 | 通过 | 7 分钟脚本、30 至 60 分钟展开顺序、故障预案和三张关键截图已入库 |
| 架构表达 | 通过 | 系统、数据模型、动态 Loop、客户端恢复、审批崩溃窗口和元数据工具图已入库 |
| 指标可追溯 | 通过 | E2 数字映射到版本化数据集、Git revision、JSON/Markdown 报告和自动化测试 |
| 简历一致性 | 通过 | AI 前端、AI 全栈和英文版本均包含数据边界与禁止扩大结论 |
| 工程回归 | 通过 | 17 个测试文件通过、2 个集成测试文件按环境跳过；63 个测试通过、7 个按环境跳过；Web 生产构建通过 |

M4 第一版达到面试交付门槛。后续更新属于演示体验和真实集成增强，不阻塞当前简历使用。
